...
// Define a nodemailer transporter for sending emails
...
// Define a route for handling forget password requests
app.post('/forgot-password', async (req, res) => {
  ...
  // Generate a reset token and set its expiration date to 1 hour from now
  const resetToken = uuidv4();
  const resetTokenExpiration = new Date(Date.now() + 60 * 60 * 1000);

  // Update the user's reset token and reset token expiration date in the database
  await User.updateOne({ email }, { resetToken, resetTokenExpiration });
...
app.post('/forgot-passwd' , async (req, res) => {
	const { email,hashedPassword } = req.body;
  
	// Find the user with the given email
	const user = await user_collection.User.findOne({ email });

	user_collection.User.findOne({ email }, (err, foundUser) => {
		if (!err && foundUser) {
			user_collection.User.updateOne(
				{ email },
				{ password: hashedPassword, resetToken: null, resetTokenExpiration: null }
			  );
			foundUser.save(function () {
				console.log(`Email sent: ${req.body.hashedPassword}`);
				res.redirect('/login');
			})
		}
	})
...
app.post('/reset-password/:resetToken', async (req, res) => {
  ...
  // Find the user with the given reset token and check if the reset token is still valid
  const user = await User.findOne({
    resetToken,
    resetTokenExpiration: { $gt: new Date() },
  });

  // If user not found or reset token is invalid, return an error
  if (!user) {
